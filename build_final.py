...
fetch('notice.json',{cache:'no-cache'}).then(function(r){return r.json()}).then(function(n){
  ...
  if(String(n.v)!==sv){
    ...
    // Force refresh for HTML updates
    if(n.force){showToast('新版本已就绪，3秒后刷新...','update');setTimeout(function(){location.reload()},3000);return}
    document.getElementById('notice-title').textContent=n.title||'';
    document.getElementById('notice-body').innerHTML=n.body||'';
    if(n.img){document.getElementById('notice-body').innerHTML+='<img src=\"'+n.img+'\" style=\"max-width:100%;border-radius:16px;margin-top:12px\">'}
